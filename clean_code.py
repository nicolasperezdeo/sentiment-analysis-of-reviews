...
#TODO describe the returned variables
...
def train_model(neural_net, X_train, y_train, top_words=5000, max_review_length=500, silent=True):
    """
    ...
    if os.path.exists(f"./models/{neural_net}.h5") and \
            os.path.exists(f"./models/{neural_net}.hist"):
        model = load_model(f"./models/{neural_net}.h5")
        history = pickle.load(open(f"./models/{neural_net}.hist.h5" 'rb'))
        if not silent:
            print(model.summary())
    else:
        # create the model
        ...
        with open(f"./models/{neural_net}.hist", 'w') as f:
            pickle.dump(history, f)
        # save the trained model for future use
        model.save(f"./models/{neural_net}.h5")
...
def plot_statistics(history, neural_network, statistic, silent=True):
    ...
    # TODO fix the y scale to the same overall value
    # TODO plot train and test error in the same plot
    ...
    plt.plot(epochs, training_stat)
    plt.plot(epochs, test_stat)
    plt.title(neural_network)
    plt.legend([f'Train {label}', f'Test {label}'], loc='upper right')
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
...
if __name__ == '__main__':
    ...
    # plot the accuracies for both networks
